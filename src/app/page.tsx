...
import {
  BookOpen, Star, Users, Globe, Phone, Mail, MessageCircle,
  ArrowRight, GraduationCap, Clock, Menu, X,
} from 'lucide-react';
...
export default function HomePage(): React.ReactNode {
  ...
  const translations = useMemo(() => ({
    en: {
      ...
      whyUs: 'Why Choose Dar Al Huda?', ourCourses: 'Our Courses',
      quranPrograms: 'Quran Programs', islamicStudies: 'Islamic Studies',
      ...
      getStarted: 'Contact Us Now', howToRegister: 'How to Register',
    ...
    am: {
      home: 'መነሻ', about: 'ስለ እኛ', courses: 'ኮርሶች', contact: 'አድራሻ',
      login: 'ግባ', dashboard: 'ዳሽቦርድ',
      ...
      whyUs: 'ለምን ዳር አል-ሁዳ?', ourCourses: 'የእኛ ኮርሶች',
      quranPrograms: 'የቁርአን ፕሮግራሞች', islamicStudies: 'ኢስላሚክ ትምህርቶች',
      availableTimes: 'የሚገኙ የትምህርት ሰዓቶች',
      readyToStart: 'የቁርአን ጉዞዎን ለመጀመር ዝግጁ ነዎት?',
      readyDesc: 'ለመመዝገብ ያግኙንና ከብቁ መምህራን ጋር ቁርአንን መማር ይጀምሩ።',
      getStarted: 'አሁን ያግኙን', howToRegister: 'እንዴት መመዝገብ እንደሚቻል',
      ...
      quickLinks: 'ፈጣን አገናኞች', contactUs: 'አድራሻ', followUs: 'ይከታተሉን',
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20 md:h-24">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={toggleMobileMenu} className="lg:hidden" aria-label="Toggle menu">
                {mobileMenuOpen ? (
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white border-t">
            <div className="px-4 py-4 space-y-3">
              {['home', 'courses', 'about', 'contact'].map((section) => (
                <Link key={section} href={section === 'home' ? '/' : `#${section}`}
                  className="block py-2 text-text font-medium" onClick={() => setMobileMenuOpen(false)}>
                  {t[section as keyof typeof t]}
                </Link>
              ))}
            </div>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary via-primary to-secondary">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32">
          <div className="text-center">
            <div className="mb-8">
              <p className="text-3xl sm:text-4xl lg:text-5xl font-arabic text-accent mb-4 leading-relaxed">{t.quranVerse}</p>
              <p className="text-lg sm:text-xl text-white/80 italic">{t.quranTranslation}</p>
              <p className="text-sm text-white/60 mt-2">{t.quranReference}</p>
            </div>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">{t.heroTitle}</h1>
            <p className="text-lg sm:text-xl text-white/80 max-w-3xl mx-auto mb-10">{t.heroSubtitle}</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 mt-16">
              {heroStats.map((stat, index) => (
                <div key={index} className="text-center">
                  <p className="text-3xl sm:text-4xl font-bold text-white">{stat.value}</p>
                  <p className="text-white/70 text-sm mt-1">{language === 'en' ? stat.labelEn : stat.labelAm}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="bg-white rounded-3xl p-8 shadow-2xl border border-primary/10">
              <div className="text-center">
                <p className="text-4xl sm:text-5xl font-arabic text-primary mb-6 leading-relaxed">{t.quranVerse2}</p>
                <p className="text-xl text-text italic mb-4">{t.quranTranslation2}</p>
                <Badge variant="outline" className="text-primary border-primary/30">{t.quranReference2}</Badge>
              </div>
            </div>
            <div className="bg-gradient-to-br from-accent/10 to-primary/10 rounded-3xl p-8 border border-accent/20">
              <div className="text-center">
                <Star className="h-12 w-12 text-accent mx-auto mb-4" />
                <p className="text-xl sm:text-2xl text-text italic mb-4 leading-relaxed">{t.hadith}</p>
                <Badge className="bg-accent/20 text-accent border-0">{t.hadithReference}</Badge>
              </div>
            </div>
          </div>
        </div>
            ...
            {whyUsItems.map((item, index) => (
              <Card key={index} className="hover:shadow-xl transition-all duration-300 group border-0">
                <CardContent className="pt-6 text-center">
                  ...
                  <h3 className="font-semibold text-lg mb-2">{language === 'en' ? item.titleEn : item.titleAm}</h3>
                  <p className="text-sm text-gray-500">{language === 'en' ? item.descEn : item.descAm}</p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-16">
            <h3 className="text-2xl font-bold text-text mb-8 flex items-center gap-2">
              <BookOpen className="h-6 w-6 text-primary" />{t.quranPrograms}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {quranCourses.map((course) => (
                <Card key={course.id} className="hover:shadow-xl transition-all duration-300 group border-0 overflow-hidden">
                  ...
                  <CardContent className="pt-6">
                    ...
                    <h3 className="font-semibold text-lg mb-2">{getCleanTitle(course)}</h3>
                    <p className="text-sm text-gray-500">{getCleanDescription(course)}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
          ...
          <div>
            <h3 className="text-2xl font-bold text-text mb-8 flex items-center gap-2">
              <Star className="h-6 w-6 text-accent" />{t.islamicStudies}
            </h3>
      ...
      <section className="py-20 bg-gradient-to-r from-primary to-secondary">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <a href="https://t.me/jemil1456" target="_blank" rel="noopener noreferrer">
            <Button size="lg" className="bg-accent hover:bg-accent/90 text-white text-lg px-12 py-6 rounded-full shadow-xl">
              {t.getStarted}<ArrowRight className="h-5 w-5 ml-2" />
            </Button>
          </a>
        </div>
      </section>
    ...
    />
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm text-gray-400">
                <li className="flex items-center gap-2"><Phone className="h-4 w-4" /> +251 91 460 0349</li>
                <li className="flex items-center gap-2"><Phone className="h-4 w-4" /> +251921757379</li>
                <li className="flex items-center gap-2"><Mail className="h-4 w-4" /> mohammedjemal433@gmail.com</li>
                <li className="flex items-center gap-2"><MessageCircle className="h-4 w-4" /> @jemil1456</li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold mb-4">{t.followUs}</h4>
              <div className="space-y-2 text-sm text-gray-400">
                <a href="https://t.me/+--kQbhPL5J-cyMzU0" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-white transition-colors">
                  <MessageCircle className="h-4 w-4" /> Telegram Group
                </a>
                <a href="https://t.me/jemil1456" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-white transition-colors">
                  <MessageCircle className="h-4 w-4" /> Telegram: @jemil1456
                </a>
                <a href="https://tiktok.com/@jemil123gc" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-white transition-colors">
          ...
          <div className="border-t border-gray-800 pt-8 text-center">
            <p className="text-gray-500 text-sm">© {new Date().getFullYear()} Dar Al Huda Academy. {t.rights}</p>
            <p className="text-gray-600 text-xs mt-2">Developed by Abubeker Oumer</p>
          </div>
        </div>
      </footer>
