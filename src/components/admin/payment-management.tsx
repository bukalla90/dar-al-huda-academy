...
export function PaymentManagement({ payments, language }: PaymentManagementProps) {
  ...
  async function handleStatusUpdate(paymentId: string, newStatus: PaymentStatus): Promise<void> {
    ...
    try {
      ...
      // Optimistic update would go here with proper state management
    } catch (error) {
        ...
        {payments.map((payment) => {
          ...
          return (
            ...
            >
              ...
              {payment.notes && (
                <p className="text-sm text-gray-600 mb-3">{payment.notes}</p>
              )}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  <DropdownMenuItem onClick={() => handleStatusUpdate(payment.id, 'PAID')}>
                    <Check className="w-4 h-4 mr-2 text-green-600" />
                    {language === 'en' ? 'Paid' : 'ተከፍሏል'}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleStatusUpdate(payment.id, 'UNPAID')}>
                    <X className="w-4 h-4 mr-2 text-gray-600" />
                    {language === 'en' ? 'Unpaid' : 'አልተከፈለም'}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleStatusUpdate(payment.id, 'PARTIAL')}>
                    <AlertTriangle className="w-4 h-4 mr-2 text-yellow-600" />
                    {language === 'en' ? 'Partial' : 'በከፊል'}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleStatusUpdate(payment.id, 'OVERDUE')}>
          ...
          <tbody>
            {payments.map((payment) => (
              <tr key={payment.id} className="border-b">
                <td className="py-3 px-4">{payment.student?.fullName}</td>
